...
% Only for non-conditional simulation.
% Inputs:
...
function write_dimension_parfile(filename,gx,gy)
fid =  fopen(filename,'w');
fprintf(fid,'                  Parameters for SGSIM\r\n');
fprintf(fid,'                  ********************\r\n');
fprintf(fid,'                                        \r\n');
fprintf(fid,'START OF PARAMETERS:\r\n');
fprintf(fid,'hard_data_ifgu.txt                            -file with data\r\n');
...
fprintf(fid,'1                             -  consider ref. dist (0=no, 1=yes)\r\n');
fprintf(fid,'hist_ifgu.txt                  -  file with ref. dist distribution\r\n');
...
fprintf(fid,'sgs_ifgu.txt                     -file for simulation output\r\n');
...
fprintf(fid,strcat(num2str(ceil(rand(1)*10000)),'    0.5    1.0              -nz,zmn,zsiz\r\n'));
...
fprintf(fid,'         100.0  50.0  1.0     -a_hmax, a_hmin, a_vert1\r\n');
...
% fgetl(fid);
% fprintf(fid,num2str(ceil(rand(1)*10000)));
% fclose(fid)