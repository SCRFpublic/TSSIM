%gets core data located in the simulation area
%function [length_core core_type type_contact well_last_data datai dataj...
%    pos_data] = get_core_data(hard_data_location,dirname,g)
% inputs: 
%     hard_data_locations: matrix obtained using read_harddata.
%     dirname: core locations obtained using get_dirname.
%     g: grid dimensions.
% Outputs:
...
function [length_core core_type type_contact well_last_data datai dataj...
    pos_data] = get_core_data(hard_data_location,dirname,g)
pos_data = find(hard_data_location(:,:,1) ~= 0);
pos_data = pos_data(1);
