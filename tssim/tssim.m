...
%==== based simulation. The README file in the Documentation directory ====
%==== explains the inputs and outputs. Run 'tssim' without any arguments ==
%==== to see an example with default input parameters. Uses functions   ===
%==== from the Matlab Image Processing toolbox.                         ===
%==== Parameters can be changed by editing tssim.m                      ===
...
int_dep_rate = 5e-5;%Avg. depos. rate [m/yr] for time between lobe deposition 
nlobe = 5;
trend_map_range = 0.3;
s_source_windows = 60;
...
%----------- Flags --------------------------------------------------------
show_plots = true;
keep_plots_after_iteration = false;
consider_erosion = true; max_erosion_fraction = 0.2;
add_noise = false; % if true, calls 'sgsim'
add_noise_ifgu = false;
smooth_surface = true; lmv.smoothing = 11;
conditional = false; n_wells = 3;
%--------------------------------------------------------------------------
...
%----------- Reading CDFs from the text files --------------------------------
...
if show_plots == true
    [nx, ny, nz] = size(surface_cube);
    for k = 1:nz, surf(surface_cube(:,:,k)), shading flat, hold on; end
    axis off; camproj p; view(120, 45);
end
%toc;
%---------- End of loop ---------------------------------------------------
%--------------------------------------------------------------------------
